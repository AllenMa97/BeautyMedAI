...
class TimeLocationService:
    ...
    def warmup(self) -> bool:
        ...
        if self._initialized:
            logger.info("时间地理位置服务已预热，跳过")
            return True
        
        logger.info("正在预热时间地理位置服务...")
        try:
            ...
            logger.info(f"时间地理位置服务预热完成: {time_info.get('formatted_time', 'N/A')}")
            return True
        except Exception as e:
            logger.warning(f"时间地理位置服务预热失败: {e}")
            return False
    ...
    def _get_time_from_official_sources(self) -> Optional[Dict[str, str]]:
        ...
        official_time_sources = [
            'https://www.ntsc.ac.cn',  # 中国科学院国家授时中心
            'https://www.beijing-time.org',  # 北京时间官网
            'https://time.tianqi.com',  # 天气网时间服务
            ...
            'https://www.qq.com',  # 腾讯
            'https://www.btime.com',  #北京时间
        ]
        
        # 用短 timeout，快速失败后用本地时间
    ...
    def get_location_info(self, ip_address: Optional[str] = None) -> Dict[str, str]:
        """
        获取地理位置信息（在没有商用API的情况下，使用基础方法）
        """
        # 如果没有IP地址，则返回默认信息
        if not ip_address:
            return {
                "country": "中国",
                "province": "广东省",  # 默认省份，可以根据实际情况调整
                "city": "广州市",      # 默认城市
    ...
    def get_context_info(self, ip_address: Optional[str] = None) -> Dict[str, any]:
        ...
        context_info = {
            ...
            # "combined_context": f"当前时间是{time_info['formatted_time']}，您位于{location_info['location_desc']}。",
...
# 创建全局实例
time_location_service = TimeLocationService()